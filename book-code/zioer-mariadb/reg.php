...
<?php
	...
	@$repassword = stripslashes(trim($_POST['retxtLoginPwd']));//密码
	
	if ($username <> "" && $password <> "") 
	{
		$user = new User();
		$ret = $user->checkUser($username);
		if ($ret>0)
		{
		...
		}else
		{
			...
			if ($ret>0)
			{
				header("Location: default.php?k=注册成功"); 
				exit;
    ...
    <div class="dialog">
        <div class="block">
            ...
            <div class="block-body">
                <form id="loginform" action="reg.php" method="post" class="form-vertical">
                    <label>用户名</label>
	                    <input type="text" ID="txtLoginName" name="txtLoginName"  MaxLength="18" style="width:97%" placeholder="Username" >
                    <label>密码</label>
    	                <input type="password" ID="txtLoginPwd" name="txtLoginPwd"  TextMode="Password" MaxLength="18" style="width:97%" placeholder="Password" >
					<label>密码确认</label>
    	                <input type="password" ID="retxtLoginPwd" name="retxtLoginPwd"  TextMode="Password" MaxLength="18" style="width:97%" placeholder="RePassword" >
					<div style="text-align:left;width:70px;float:left">
                        <input name="btn" type="image" src="images/backbtn.png" width="58px" height="30px" border="0" onClick="window.location='default.php';return false;">
                    </div>
                    <div style="text-align:right;width:70px;float:right">
                        <input name="imgbtn" type="image" src="images/regbtn.png" width="58px" height="30px" border="0" onClick="sumbitLogin();return false;">
                    </div>
...
<script type="text/javascript">
    ...
    function sumbitLogin() {
        ...
        if (loginName == "") {
            ...
            var repassWord = document.getElementById("retxtLoginPwd").value;
			if (passWord == "") {
                ShowDialog("请输入密码!");
